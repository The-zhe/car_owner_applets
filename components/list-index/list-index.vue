	...
	<view class="uni-indexed">
		<scroll-view :scroll-into-view="scrollViewId" :style="{ height: winHeight + 'px' }" class="uni-indexed__list" scroll-y>
			<block v-for="(list, idx) in lists" :key="idx">
				...
				<view v-if="list.items && list.items.length > 0 && list.key=='TOP'" class="uni-list-img">
					<view v-for="(item, index) in list.items" :key="index" class="uni-list-item-img" hover-class="uni-list-item--hover">
						<view class="uni-list-item__container-img" @click="onClick(idx, index)">
							<image style="width:60rpx;height:60rpx" :src="item.name.img"></image>
							<view class="uni-list-item__content-img-text">{{ item.name.name }}</view>
						</view>
					</view>
				</view>
				<view v-if="list.items && list.items.length > 0 && list.key!='TOP'" class="uni-list">
					<view v-for="(item, index) in list.items" :key="index" class="uni-list-item" hover-class="uni-list-item--hover">
						<view class="uni-list-item__container" @click="onClick(idx, index)">
							...
							<image style="width:60rpx;height:60rpx" :src="item.name.img"></image>
							<view class="uni-list-item__content" style="text-align: left;float: left;">{{ item.name.name }}</view>
						</view>
					</view>
				</view>
			</block>
