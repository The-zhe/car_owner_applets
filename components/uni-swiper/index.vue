<template>
	<view class="u-swiper-wrap" :style="{
		...
		<swiper @change="change" @animationfinish="animationfinish" :interval="interval" :circular="circular" :duration="duration"
		 :autoplay="autoplay" :previous-margin="effect3d ? effect3dPreviousMargin + 'rpx' : '0'" :next-margin="effect3d ? effect3dPreviousMargin + 'rpx' : '0'"
		 :indicator-dots='false' :style="{
				height: height + 'rpx',
			}">
			<swiper-item class="u-swiper-item" v-for="(item, index) in list" :key="index">
				<view class="u-list-image-wrap" @tap.stop.prevent="listClick(index)" :class="[current != index ? 'u-list-scale' : '']"
				 :style="{
						borderRadius: `${borderRadius}rpx`,
					...
					<image class="u-swiper-image" :src="item[name]" :mode="imgMode"></image>
						<view class="text-box top" style="top:30rpx;">
							<view class="price"><text :class="[effect3d&&current==index?'big-big':'big']">{{item.price}}</text>元</view>
							<view class="man" :class="[effect3d&&current==index?'big-height':'height']">{{item.man}}</view>
						</view>
						<view class="footer text-box" style="bottom:20rpx;">
							<view class="" :class="effect3d&&current==index?'footer-name-big':'footer-name'">{{item.quan}}</view>
				...
				bottom: indicatorPos == 'bottomLeft' || indicatorPos == 'bottomCenter' || indicatorPos == 'bottomRight' ? '-38rpx' : 'auto',
...
<style lang="scss" scoped>
	...
	.u-indicator-item-rect-active {
		background-color: rgba(230,183, 131, 1);
	}
