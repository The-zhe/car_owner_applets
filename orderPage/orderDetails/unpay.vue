	...
	<view class="unpay">
		<view class="unpay_type">
			<view class="type_title">
				...
				<view class="type_time">预计完成时间:{{payList.orderMaster.deliveryTime | timeFormat}}</view>
			</view>
		...
		<view class="unpay_total">
			<view class="total">
				<view class="total_price">合计：￥{{ payList.orderMaster.totalPrice | NumFormat}}</view>
				<view class="total_coupon">优惠：￥{{ coupon }}</view>
			</view>
			<view class="pay_for" @click="pay()">支付</view>
		</view>
	</view>
</template>

<script>
	import {wechatPay} from '../../apis/api.js'
export default {
	props: ['mainList'],
	data() {
		return {
	...
	methods: {
		...
		pay(){
			this.dataFrom.openId = uni.getStorageSync('userId')
			...
			wechatPay(this.dataFrom).then(res => {
				...
				uni.requestPayment({
					...
					signType: wxMsg.signType,
					paySign: wxMsg.paySign,
